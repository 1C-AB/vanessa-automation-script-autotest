...
**Запуск тестирования**
Тестирование запускается через файл RunScript.PS1 или командную строку: ПутьКИсполнителю "ПутьКСрипту" ПутьКНастройкам ПараметрОбновления, где
...
Если нужно добавить [новый клиент тестирования](#_добавление_нового_клиента) или [изменить настройки VA](#_изменение_настроек) необходима настройка *менеджера, клиента* тестирования, 
а также внешняя обработка VA (.epf не сингл версия) и *расширение VA* (.cfe) последние версии которых можно найти тут  <https://pr-mex.github.io/vanessa-automation/dev/> .
