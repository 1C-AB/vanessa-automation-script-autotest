**Подготовка и запуск скриптов тестирования**
...
Перед запуском тестирования необходимо настроить клиент тестирования.

1. В клиенте тестирования необходимо подключчить расширение для тестирования, актуальную версию необходимо скачать <https://pr-mex.github.io/vanessa-automation/dev/>
...
* ПутьКНастройкам - D:\GitHub\vanessa-automation-script-autotest\Scripts\Settings.txt
...
1.Настраиваем базу которая будет выступать как менеджер тестирования в режим запуска как менеджер тестирования.Для этого в параметрах запуска указываем ключ `/TestManager`
...
2. Запускаем менеджер тестирования и открываем как файл внешнюю обработку. 
3. Ожидаем полной загрузки: появления панели задач и пунктов меню. Загружаем настройки из папки **VaParams** 
...
3. На правой панели настроек выбираем «Клиенты тестирования». Если в списке клиентов нет нужного, добавляем его:
  ...
  2. Жмем «Сохранить»
     ...
     ![Aspose Words 7ef5f5f2-ab07-47b8-9392-69f1c249303d 003](https://github.com/user-attachments/assets/68630124-cdff-4cdc-ac85-5e6752c345aa)
